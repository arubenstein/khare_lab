...
#CREATEXML
cp /home/arubenstein/mean_field/xml'/mutrelax'$xml$pdb'dummy.xml' $path/$sequence".xml"
...
if [ $xml == "cat" ]; then
                ...
                const_file_arg_2="/home/arubenstein/mean_field/pdbs/ancillary_files/"$scorefxn"/"$pdb"_"$index"_sc.cst"
else
                const_file_arg_1="-constraints:cst_file /home/arubenstein/mean_field/pdbs/ancillary_files/"$scorefxn"/"$pdb"_"$index"_sc.cst"
                const_file_arg_2="-enzdes:cstfile /home/arubenstein/mean_field/pdbs/ancillary_files/"$scorefxn"/"$pdb"cstfile.txt"
fi

#MUTATERUN
if [ ! `ls mut_*.pdb 2>/dev/null | wc -l` -eq 10 ]
then
	nohup nice /home/arubenstein/Rosetta/main/source/bin/rosetta_scripts.static.linuxgccrelease -nstruct 10  -jd2:ntrials 1 -parser:protocol $path/$sequence.xml -database /home/arubenstein/Rosetta/main/database/ $const_file_arg_1 $const_file_arg_2 -out::prefix mut_ -s $path/$sequence".pdb" -run:preserve_header -overwrite @/home/arubenstein/mean_field/pdbs/ancillary_files/enzflags $extra_res_fa > design.log
fi
